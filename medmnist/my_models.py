...
class basic_block_2(nn.Module):
	def  __init__(self,in_channels,out_channels):
		super(basic_block_2,self).__init__()
...
class resnet18(nn.Module):
	...
	def __init__(self, inchannels, numclass):
		...
		self.layer1 = nn.Sequential(
			nn.Conv2d(inchannels,64,kernel_size=3,stride=2,padding=3),   #3还是7
			nn.BatchNorm2d(64),
			nn.ReLU(),
		...
		self.reslayer = nn.Sequential(
			basic_block_1(64),
			basic_block_1(64),
			basic_block_2(64,128),
			basic_block_1(128) ,
			basic_block_2(128,256),
			basic_block_1(256),
			basic_block_2(256,512),
			basic_block_1(512)
			)
...
class resnet50(nn.Module):
	def __init__(self, inchannels, numclass):
		...
		self.layer1 = nn.Sequential(
			nn.Conv2d(inchannels,64,kernel_size=3,stride=2,padding=3),
			nn.BatchNorm2d(64),
			nn.ReLU(),
			nn.MaxPool2d(kernel_size=3,stride=2,padding=1)
			)
		...
		self.resnet1 = nn.Sequential(
			bottleneck_1(64,[64,64,256],1),
			bottleneck_2(256,[64,64,256]),
			bottleneck_2(256,[64,64,256])
			)
		self.resnet2 = nn.Sequential(
			bottleneck_1(256,[128,128,512],2),
			bottleneck_2(512,[128,128,512]),
			bottleneck_2(512,[128,128,512]),
			bottleneck_2(512,[128,128,512])
			)
		self.resnet3 = nn.Sequential(
			bottleneck_1(512,[256,256,1024],2),
			bottleneck_2(1024,[256,256,1024]),
			bottleneck_2(1024,[256,256,1024]),
			bottleneck_2(1024,[256,256,1024]),
			bottleneck_2(1024,[256,256,1024]),
			bottleneck_2(1024,[256,256,1024])
		...
		self.resnet4 = nn.Sequential(
			bottleneck_1(1024,[512,512,2048],2),
			bottleneck_2(2048,[512,512,2048]),
			bottleneck_2(2048,[512,512,2048])
			)
		self.avgpool = nn.AvgPool2d(2,2,padding=1)
